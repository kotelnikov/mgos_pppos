...
enum mgos_pppos_state {
  ...
  PPPOS_RUN = 11,
  PPPOS_CLOSING = 13,
  PPPOS_BEGIN_COMMAND_MODE = 14,
  PPPOS_COMMAND_POLL = 15,
  PPPOS_COMMAND_MODE_RESP = 16,
};
...
struct mgos_pppos_data {
  ...
  bool baud_ok, cmd_mode, stay_cmd_mode;
  int attempt;
  bool try_cops, cops_set;
  double creg_start;
  mgos_timer_id poll_timer_id, cmd_delay_timer_id;
...
static void mgos_pppos_dispatch_once(struct mgos_pppos_data *pd) {
  ...
  switch (pd->state) {
    case PPPOS_IDLE: {
      ...
      if (pd->cmd_delay_timer_id != MGOS_INVALID_TIMER_ID) {
        mgos_clear_timer(pd->cmd_delay_timer_id);
        pd->cmd_delay_timer_id = MGOS_INVALID_TIMER_ID;
      }
      if (pd->cfg->pwr_gpio >= 0) {
        mgos_gpio_write(pd->cfg->pwr_gpio, pd->cfg->pwr_act);
      }
    ...
    case PPPOS_INIT: {
      if (pd->cfg->apn == NULL) {
        if (!mgos_sys_config_get_pppos_stay_cmd_mode()) {
      ...
      if (!pd->cmd_mode && pd->deadline == 0) {
        ...
        if (pd->cmds != NULL) {
          ...
          if (!mgos_sys_config_get_pppos_stay_cmd_mode()) {
            ...
            pd->cmd_success_state = PPPOS_BEGIN_COMMAND_MODE;
          }
    ...
    case PPPOS_SETUP: {
      ...
      if (!mgos_sys_config_get_pppos_stay_cmd_mode()) {
        LOG(LL_INFO, ("Connecting (UART%d, APN '%s')...", pd->cfg->uart_no, (apn ? apn : "")));
      }
      else {
        LOG(LL_INFO, ("Initializing GSM mode (UART%d)...", pd->cfg->uart_no));
      }
      ...
      if (mgos_sys_config_get_pppos_stay_cmd_mode()) {
        add_cmd(pd, mgos_pppos_atd_cb, 0, "AT+CMGF=1");
      }
      else {
    ...
    case PPPOS_CMD: {
      ...
      if (!pd->stay_cmd_mode) {
        ...
        mgos_pppos_set_state(pd, PPPOS_COMMAND_MODE_RESP);
      }
      break;
    }
    case PPPOS_CMD_RESP: {
    ...
    case PPPOS_BEGIN_COMMAND_MODE: {
      pd->stay_cmd_mode = true;
      struct mgos_pppos_info_arg info_arg = {
          .info = pd->ati_resp,
          .imei = pd->imei,
          .imsi = pd->imsi,
          .iccid = pd->iccid,
      ...
      mgos_event_trigger(MGOS_PPPOS_GSM_READY, &info_arg);
      mgos_pppos_set_state(pd, PPPOS_COMMAND_POLL);
      break;
    }
    case PPPOS_COMMAND_MODE_RESP: {
      const double now = mgos_uptime();
      struct mgos_pppos_cmd *cur_cmd = &pd->cmds[pd->cmd_idx];
      if (now > pd->deadline) {
        LOG(LL_INFO, ("Command timed out: %s", cur_cmd->cmd));
      ...
      if (pd->data.len == 0) break;
      struct mg_str buf = mg_mk_str_n(pd->data.buf, pd->data.len);
      bool cmd_done = false;

      char *eol;
      char *text_request = (char *) mg_strchr(buf, '>');
      char *cr = (char *) mg_strchr(buf, '\r');
      char *lf = (char *) mg_strchr(buf, '\n');

      if (text_request != NULL) {
        eol = text_request + 1;
      }
      else {
        if (cr == NULL && lf == NULL) return;
        else if (cr != NULL && lf == NULL) eol = cr; 
        else if (cr == NULL && lf != NULL) eol = lf;
        else eol = MIN(cr, lf);
      }
      *eol = '\0';

      size_t line_len = eol - pd->data.buf;
      if (line_len == 0) return;
      struct mg_str line = mg_mk_str_n(pd->data.buf, line_len);

      mgos_msleep(100);
      if ( mg_strstr(line, mg_mk_str("+CMGL:")) != NULL) { cmd_done = true; }
      else if ( mg_strstr(line, mg_mk_str("OK")) != NULL ) { cmd_done = true; }
      else if ( mg_strstr(line, mg_mk_str(">")) != NULL ) { cmd_done = true; }
      else if ( mg_strstr(line, mg_mk_str("ERROR")) != NULL ) { cmd_done = true; }

      if (cmd_done) {
        pd->deadline = 0;
        if (cur_cmd->cb != NULL) {
          cur_cmd->cb(cur_cmd->cb_arg, true, line);
        }
        pd->cmd_idx++;
        if (pd->cmd_idx >= pd->num_cmds ||
            pd->cmds[pd->cmd_idx].cmd == NULL) {
    ...
    case PPPOS_COMMAND_POLL: {
      if (pd->data.len == 0) break;
      struct mg_str buf = mg_mk_str_n(pd->data.buf, pd->data.len);
      
      static int sms_mem_cell = 0;
      char *sol;
      char *eol;
      char *cr = (char *) mg_strchr(buf, '\r');
      char *lf = (char *) mg_strchr(buf, '\n');
      char *sms_end = 
        mg_strstr(buf, mg_mk_str("\r\n\r\n")) != NULL ? mg_strstr(buf, mg_mk_str("\r\n\r\n")) + 3 : NULL;
        // mg_strstr(buf, mg_mk_str("\r\n\r\nOK\r\n")) != NULL ? mg_strstr(buf, mg_mk_str("\r\n\r\nOK\r\n")) + 7 : NULL;

      if (sms_end != NULL) eol = sms_end;
      else if (cr == NULL && lf == NULL) return;
      else if (cr != NULL && lf == NULL) eol = cr; 
      else if (cr == NULL && lf != NULL) eol = lf;
      else eol = MIN(cr, lf);
      *eol = '\0';
      ...
      mgos_msleep(100);
      if      ( (sol = mg_strstr(line, mg_mk_str("+CLIP:"))) != NULL ) {
        if (mgos_sys_config_get_pppos_decline_calls()) {
          mgos_pppos_at_cmd(uart_no, "ATH");
        }
        struct mgos_pppos_call_arg call_arg;
        sscanf(sol, "+CLIP: \"%[^\"]", call_arg.cid);
        LOG(LL_VERBOSE_DEBUG, ("\r\n--- PPPOS Call data ---\r\nCID: %s\r\n--- PPPOS Call data ---", call_arg.cid));
        mgos_event_trigger(MGOS_PPPOS_CALL, &call_arg);
        mbuf_remove(&pd->data, line_len + 1);
      }
      else if ( (sol = mg_strstr(line, mg_mk_str("+CMTI:"))) != NULL ) {
        int mem_cell = 0;
        sscanf(sol, "+CMTI: \"%*[^\"]\",%d", &mem_cell);
        if (mem_cell) {
          sms_mem_cell = mem_cell;
          LOG(LL_VERBOSE_DEBUG, ("\r\n--- PPPOS Sms data ---\r\nMEM_CELL: %d\r\n--- PPPOS Sms data ---", mem_cell));
          char *read_sms_cmd;
          mg_asprintf(&read_sms_cmd, 0, "AT+CMGR=%d", mem_cell);
          mgos_pppos_at_cmd(uart_no, read_sms_cmd);
          free(read_sms_cmd);
        }
        mbuf_remove(&pd->data, line_len + 1);
      }
      else if ( (sol = mg_strstr(line, mg_mk_str("+CMGR:"))) != NULL ) {
        if (sms_end == NULL) break;
        struct mgos_pppos_sms_arg sms_arg;
        sscanf(sol, "+CMGR: \"%*[^\"]\",\"%[^\"]\",\"\",\"%[^\"]\"\r\n%[^\r\n]\r\n\r",
          sms_arg.cid, sms_arg.timestamp, sms_arg.data);
        LOG(LL_VERBOSE_DEBUG, ("\r\n--- PPPOS SMS data ---\r\nCID: %s\r\nTEXT: %s\r\nDATA: %s\r\n--- PPPOS SMS data ---",
          sms_arg.cid, sms_arg.timestamp, sms_arg.data));
        if (mgos_sys_config_get_pppos_delete_sms() && sms_mem_cell) {
          char *dell_sms_cmd = NULL;
          sprintf(sms_arg.id, "%d", sms_mem_cell);
          mgos_event_trigger(MGOS_PPPOS_SMS, &sms_arg);
          mg_asprintf(&dell_sms_cmd, 0, "AT+CMGD=%d", sms_mem_cell);
          mgos_pppos_at_cmd(uart_no, dell_sms_cmd);
          sms_mem_cell = 0;
          free(dell_sms_cmd);
        }
        mbuf_remove(&pd->data, line_len + 1);
      }
      else if ( (sol = mg_strstr(line, mg_mk_str("+CMGL:"))) != NULL ) {
        if (sms_end == NULL) break;
        struct mgos_pppos_sms_arg sms_arg;
        sscanf(sol, "+CMGL: \"%*[^\"]\",\"%[^\"]\",\"\",\"%[^\"]\"\r\n%[^\r\n]\r\n\r",
          sms_arg.cid, sms_arg.timestamp, sms_arg.data);
...
static void cmd_delay_cb(void *arg) {
  struct mgos_pppos_data *pd = (struct mgos_pppos_data *) arg;
  if (pd->data.len  > 1) return;
  mgos_clear_timer(pd->cmd_delay_timer_id);
  pd->cmd_delay_timer_id = MGOS_INVALID_TIMER_ID;
...
bool mgos_pppos_run_cmds(int if_instance, const struct mgos_pppos_cmd *cmds) {
  ...
  if (pd->data.len != 0) {
    ...
    pd->cmd_delay_timer_id = mgos_set_timer(1000, true, cmd_delay_cb, pd);
    return true;
  }
  pd->cmd_success_state = pd->state;
  pd->cmd_error_state = pd->state;
...
bool mgos_pppos_create(const struct mgos_config_pppos *cfg, int if_instance) {
  ...
  pd->state = PPPOS_IDLE;
  if (pd->cfg->pwr_gpio >= 0) {
    mgos_gpio_setup_output(pd->cfg->pwr_gpio, pd->cfg->pwr_act);
  }
  if (pd->cfg->dtr_gpio >= 0) {
...
bool mgos_pppos_init(void) {
  ...
  if (mgos_sys_config_get_pppos_connect_on_startup() ||
      mgos_sys_config_get_pppos_stay_cmd_mode()) {
